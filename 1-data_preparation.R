...
share_imp <- lapply(share_imp, function(x){
  ...
  row.names(data) <- NULL
  incomedf <- lapply(unique(data$country), function(c){
    codf <- data[data$country == c, ]
    q <- quantile(codf["thinc"], probs = seq(0, 1, 1/3), na.rm = TRUE)
    income <- cbind(codf, list("l" = q[2], "m" = q[3], "h" = q[4]))})
  res <- Reduce("rbind", incomedf)
...
share <- sapply(1:8, function(x){
  df <- merge(share[[x]], id, by=c("mergeid","wave"), all.x=T)
  df1 <- df[!is.na(df$wave),]
  if(mean(df1$wave)==3){
    df1$dn004_ <- with(df1, ifelse(first==3, sl_ac013_1, NA))
    df1$dn005c <- with(df1, ifelse(first==3, sl_ac014c_1_iso, NA))
    df1$dn006_ <- ifelse(
      !is.na(df1$sl_ac013_1) & df1$sl_ac013_1 == 1, NA, ifelse(
        df1$first != 3, NA, apply(sapply(1:29, function(x){
          ac006 <- df1[grep("ac006",names(df1))]
          ac013 <- df1[grep("ac013",names(df1))]
          ifelse(!is.na(ac013[,x]) & ac013[,x]==1, ac006[,x], NA)
        }), 1, function(y)ifelse(all(is.na(y)), NA, min(y, na.rm = T)))))
  } 
  df1[-grep("^sl",names(df1))]
}, simplify=F)

share <- sapply(1:8, function(x){
  df <- merge(share[[x]], share_imp[[x]], by=c("mergeid","wave"), all.x=T)
  return(df)
}, simplify=FALSE)
...
df1 <- Reduce("rbind", share)
df1 <- df1[order(df1$mergeid, df1$wave),]
df1$dn004_1 <- df1$dn004_
i1 <- which(names(df1) %in% c("dn004_1","dn005c","dn006_"))
df1[i1] <- apply(df1[i1], 2, function(x) ifelse(!is.na(x) & x<0, NA, x))
df1[i1] <- apply(df1[i1], 2, function(x) fill_down(x, df1$mergeid))
df1 <- df1[order(df1$mergeid, -df1$wave),]
df1[i1] <- apply(df1[i1], 2, function(x) fill_down(x, df1$mergeid))
df1$dn004_1[df1$dn004_1 == 1 & !is.na(df1$dn005c)] <- 5
df1 <- df1[order(df1$mergeid, df1$wave),]
id <- merge(id, df1[c("mergeid","wave","interview")])
df1 <- df1[df1$wave %in% c(1:2,4:8),]
df1 <- df1[df1$interview==1,]
rownames(df1) <- NULL
rm(share, i1)

# imm_citizenship at birth ------------------------------------------------
df1$dn004_2 <- NA
df1$dn004_2 <- ifelse(!is.na(df1$dn004_) & df1$dn004_==1, 1, df1$dn004_2)
df1$dn004_2 <- ifelse(is.na(df1$dn004_2) & 
                        !is.na(df1$dn004_) & df1$dn004_==5 & 
                        !is.na(df1$dn007_) & df1$dn007_==1 &
                        !is.na(df1$dn503_) & df1$dn503_==1, 1, df1$dn004_2)
df1$dn004_2 <- ifelse(is.na(df1$dn004_2) & 
                        !is.na(df1$dn004_) & df1$dn004_==5, 5, df1$dn004_2)

df1$dn004_2[df1$wave %in% 1:4] <- NA
df1$dn004_2 <- fill_down(df1$dn004_2, df1$mergeid)
i <- apply(table(df1$mergeid, df1$dn004_2)>0, 1, sum) > 1
df1$dn004_2[df1$mergeid %in% unique(df1$mergeid)[i]] <- NA
row.names(df1) <- NULL
rm(i)

# imm_border change -------------------------------------------------------
df1$dn004_3 <- df1$dn004_1
df1$dn004_3 <- ifelse(!is.na(df1$dn004_1) & df1$dn004_1==5 &
                        !is.na(df1$dn005c) & df1$dn005c==200 &
                        df1$country %in% c(28,63), NA, df1$dn004_3)
df1$dn004_3 <- ifelse(!is.na(df1$dn004_1) & df1$dn004_1==5 &
                        !is.na(df1$dn005c) & df1$dn005c %in% c(278,280) &
                        df1$country == 12, NA, df1$dn004_3)
df1$dn004_3 <- ifelse(!is.na(df1$dn004_1) & df1$dn004_1==5 &
                        !is.na(df1$dn005c) & df1$dn005c==810 &
                        df1$country %in% c(35,48,57), NA, df1$dn004_3)

# isced1997 ---------------------------------------------------------------
df1$isced1997_r[duplicated(df1$mergeid)] <- NA
df1$isced1997_r[!is.na(df1$isced1997_r) & df1$isced1997_r < 0] <- NA
df1$isced1997_r[!is.na(df1$isced1997_r) & df1$isced1997_r > 6] <- NA
df1$isced1997_r <- fill_down(df1$isced1997_r, df1$mergeid)
...
df1$dn014_[!is.na(df1$dn014_) & df1$dn014_ < 0] <- NA
df1$dn044_[!is.na(df1$dn044_) & df1$dn044_ < 0] <- NA
df1$dn044_[is.na(df1$dn044_)] <- 5
df1$dn014_ <- fill_down(df1$dn014_, df1$dn044_, df1$mergeid)
df1 <- df1[order(df1$mergeid, -df1$wave),]
df1$dn014_ <- fill_down2(df1$dn014_, df1$dn044_, df1$mergeid)
df1 <- df1[order(df1$mergeid, df1$wave),]
row.names(df1) <- NULL
...
df1$br001_ <- ifelse(!is.na(df1$br001_) & df1$br001_ < 0, NA, df1$br001_)
df1$br002_ <- ifelse(!is.na(df1$br002_) & df1$br002_ < 0, NA, df1$br002_)
df1$br005d1 <- ifelse(!is.na(df1$br005d1) & df1$br005d1 < 0, NA, df1$br005d1)
df1$br006_ <- ifelse(!is.na(df1$br006_) & df1$br006_ < 0, NA, df1$br006_)
df1$br006_2 <- ifelse(is.na(df1$br006_), NA, ifelse((df1$br006_ > 0), 1, 0))

df1 <- df1[order(df1$mergeid, -df1$wave),]
df1$br001_2 <- fill_down(df1$br001_, df1$mergeid)
df1$br005d1_2 <- fill_down(df1$br005d1, df1$mergeid)
df1$br006_3 <- fill_down(df1$br006_2, df1$mergeid)
df1$br002_2 <- fill_down(df1$br002_, df1$mergeid)

df1 <- df1[order(df1$mergeid, df1$wave),]
df1$br001_3 <- fill_down2(df1$br001_2, df1$mergeid)
df1$br005d1_3 <- fill_down3(df1$br005d1_2, df1$mergeid)

df1$evsmoke <- ifelse(
  (!is.na(df1$br001_3)) & (df1$br001_3 == 5), 0, ifelse( 
    (!is.na(df1$br005d1_3)) & (df1$br005d1_3 == 0), 0, ifelse(
      (!is.na(df1$br001_3)) & (df1$br001_3 == 1), 1, ifelse(
        (!is.na(df1$br005d1_3)) & (df1$br005d1_3 == 1), 1, NA))))

df1$crsmoke <- ifelse(
  (!is.na(df1$br002_2)) & (df1$br002_2 == 5), 0, ifelse( 
    (!is.na(df1$br006_3)) & (df1$br006_3 == 0), 0, ifelse(
      (!is.na(df1$br002_2)) & (df1$br002_2 == 1), 1, ifelse(
        (!is.na(df1$br006_3)) & (df1$br006_3 == 1), 1, NA))))

df1$smoke <- with(df1, ifelse(
  !is.na(evsmoke) & evsmoke==0, 0, ifelse(
    !is.na(crsmoke) & crsmoke==0, 0, ifelse(
      !is.na(crsmoke) & crsmoke==1, 1, NA))))
  
df1 <- df1[order(df1$mergeid, -df1$wave),]
df1$smoke <- fill_down(df1$smoke, df1$mergeid)
...
df1$br011_ <- ifelse(!is.na(df1$br011_) & df1$br011_ < 0, NA, df1$br011_)
df1$br012_ <- ifelse(!is.na(df1$br012_) & df1$br012_ < 0, NA, df1$br012_)
df1$br013_ <- ifelse(!is.na(df1$br013_) & df1$br013_ < 0, NA, df1$br013_)
df1$br010_ <- ifelse(!is.na(df1$br010_) & df1$br010_ < 0, NA, df1$br010_)
df1$br019_ <- ifelse(!is.na(df1$br019_) & df1$br019_ < 0, NA, df1$br019_)
df1$br039_ <- ifelse(!is.na(df1$br039_) & df1$br039_ < 0, NA, df1$br039_)
df1$br040_ <- ifelse(!is.na(df1$br040_) & df1$br040_ < 0, NA, df1$br040_)

df1$alcohol <- with(df1, ifelse(
...
df1$alcohol <- ifelse(!is.na(df1$br010_) & df1$br010_ == 7, 0, df1$alcohol)
df1$alcohol <- with(df1, ifelse(
  wave %in% 2:5 & is.na(br010_), NA, ifelse(
    ...
    1, ifelse(wave %in% 2:5, 0, df1$alcohol))))

df1$br040_ <- ifelse(!is.na(df1$br039_) & (df1$br039_ == 5), 0, df1$br040_)
df1$alcohol <- with(df1, ifelse(
  wave %in% 6:8 & !is.na(br040_) & (br040_ > 14), 1, ifelse(
    wave %in% 6:8 & is.na(br040_), NA, ifelse(
      wave %in% 6:8, 0, df1$alcohol))))

df1 <- df1[order(df1$mergeid, -df1$wave),]
df1$alcohol <- fill_down(df1$alcohol, df1$mergeid)

df1 <- df1[order(df1$mergeid, df1$wave),]
row.names(df1) <- NULL

# physical activity -------------------------------------------------------
df1$br015_ <- ifelse(!is.na(df1$br015_) & df1$br015_ < 0, NA, df1$br015_)
df1$pa <- ifelse(is.na(df1$br015_), NA, ifelse(df1$br015_==4, 0, 1))
df1 <- df1[order(df1$mergeid, -df1$wave),]
df1$pa <- fill_down(df1$pa, df1$mergeid)
df1 <- df1[order(df1$mergeid, df1$wave),]
row.names(df1) <- NULL
rm(fill_down)

# variables ---------------------------------------------------------------
df1[,3:ncol(df1)] <- apply(
  df1[,3:ncol(df1)], 2, function(x)ifelse(!is.na(x) & x<0, NA, x))
df1$age <- df1$age_int
df1$woman <- ifelse(df1$gender == 2, 1, 0)
df1$imm <- ifelse(is.na(df1$dn004_1), NA, ifelse(df1$dn004_1 == 5, 1, 0))
df1$imm2 <- ifelse(is.na(df1$dn004_2), NA, ifelse(df1$dn004_2 == 5, 1, 0))
df1$imm3 <- ifelse(is.na(df1$dn004_3), NA, ifelse(df1$dn004_3 == 5, 1, 0))
df1$origin <- NA
df1$origin[!is.na(df1$imm)&df1$imm==0] <- "Native"
df1$origin[is.na(df1$origin) & df1$dn005c %in% c(
...
df1$origin[df1$origin %in% c("NorthAmerica", "Latin")] <- "Americas"
df1$origin[df1$origin %in% c("North", "West", "South")] <- "OtherEurope"
df1$origin[df1$origin %in% c("East", "USSR")] <- "EastEurope"
df1$region <- NA
df1$region[df1$country %in% c(13,18,30,55)] <- "N"
df1$region[df1$country %in% c(11,12,14,17,20,23,31)] <- "W"
df1$region[df1$country %in% c(15,16,19,33,34,47,53,59)] <- "S"
df1$region[df1$country %in% c(28,29,32,35,48,51,57,61,63)] <- "E"
df1$region[df1$country==25] <- "israel"
df1$marry <- ifelse(is.na(df1$dn014_), NA, ifelse(df1$dn014_ %in% 1:3, 1, 0))
df1$edu <- df1$isced1997_r
df1$edu <- with(df1, ifelse(
  is.na(edu), NA, ifelse(edu %in% 0:2, 1, ifelse(edu %in% 3:4, 2, 3))))
df1$employed <- with(df1, ifelse(is.na(ep005_), NA, ifelse(ep005_ == 2, 1, 0)))
df1$cohort <- with(df1, ifelse(!is.na(imm) & imm==0, "Native", dn006_))
df1$year <- df1$int_year
df1[c("origin","cohort")] <- apply(
  df1[c("origin","cohort")], 2, function(x){
    id <- unique(df1$mergeid)[apply(table(df1$mergeid, x)>0, 1, sum) > 1]
    x[df1$mergeid %in% id] <- NA
    return(x)})
...
df1 <- merge(id, df1, all.x = TRUE)
df1$nwave <- unlist(sapply(table(df1$mergeid), function(x)1:x))

# exclusion ---------------------------------------------------------------
i1 <- df1$wave %in% c(1:2, 4:8)
i2 <- !is.na(df1$age) & df1$age >= 50
i3 <- !is.na(df1$age) & df1$age < 80
i4 <- !is.na(df1$region) & df1$region!="israel"
exclusion1 <- Reduce("rbind",sapply(
  list(TRUE,i1,i1&i2,i1&i2&i3,i1&i2&i3&i4), function(x){
    v1 <- nrow(df1[x,])
    v2 <- length(unique(df1$mergeid[x]))
    res <- cbind(v1,v2)
    names(res) <- c("v1","v2")
...
exclusion2 <- Reduce("rbind",sapply(
  1:which(names(df2)=="ncond1"), function(x){
    cond <- apply(!is.na(df2[1:x]),1,all)
    v1 <- length(df2$mergeid[cond])
    v2 <- length(unique(df2$mergeid[cond]))
    res <- cbind("person-years"=v1,"persons"=v2)
    return(res)}, simplify = F))
rownames(exclusion2) <- names(df2)[1:which(names(df2)=="ncond1")]
...
main <- df2[apply(!is.na(df2[,1:which(names(df2)=="ncond1")]),1,all),]
